...
export default function RightPanel({
	...
	const [lastAIMessage, setAIMessage] = useState(
		rawMessages[rawMessages.length - 1]?.role === 'assistant'
			? rawMessages[rawMessages.length - 1].content
			: null
	);
	const { shell, app, commands, setText, text } = useInstructionParser();
	...
	useEffect(() => {
		if (lastAIMessage) {
			setText(lastAIMessage as string);
		}
	}, [lastAIMessage, setText]);
